-- create_order()'s _cart CTE priced a cart via a LEFT JOIN against
-- product_variants, so a missing/inactive/bogus variant_id silently produced
-- a NULL variant_id/variant_name and fell back to the base product price
-- instead of failing -- narrowing the "a tampered client cart cannot express
-- a price" guarantee this function's own header comment advertises. Fix:
-- require the variant to have actually matched whenever one was requested,
-- routing that case into the same 'one or more items are unavailable'
-- exception that already covers a deactivated product.
--
-- Body is identical to the create_order() in 20250101000500_order_functions.sql
-- except the _cart CTE's `where` clause. Convention: re-declare via
-- create or replace function in a new migration rather than editing an
-- already-applied one.

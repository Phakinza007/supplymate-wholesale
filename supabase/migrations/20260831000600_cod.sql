-- Cash on delivery. The courier collects at the door, so nothing is verified
-- before dispatch -- but the order still waits for an admin to confirm it,
-- because a COD parcel that is refused costs the shop the round trip. The
-- status path is unchanged: pending -> verified -> shipped -> done.

-- EDIT PER CLIENT: flat 40 THB service fee, 30,000 THB ceiling per order.
-- A shop that does not offer COD can leave these alone; the method is only
-- reachable when brandConfig.cod.enabled is on.
create or replace function public.calc_cod_fee()
returns numeric language sql immutable as $$ select 40::numeric(12,2); $$;

create or replace function public.cod_max_total()
returns numeric language sql immutable as $$ select 30000::numeric(12,2); $$;

alter table public.orders
  add column cod_fee numeric(12,2) not null default 0 check (cod_fee >= 0);

alter table public.orders
  drop constraint orders_payment_method_check,
  add constraint orders_payment_method_check
    check (payment_method in ('bank_transfer', 'promptpay', 'cod'));

alter table public.orders
  drop constraint orders_total_identity,
  add constraint orders_total_identity
    check (total = subtotal - discount_total + shipping_fee + cod_fee + vat_total);

-- A COD order is never paid in advance, so it must never accept a slip. The
-- guard lives here rather than only in the UI: attach_payment_slip is a
-- SECURITY DEFINER RPC the browser calls directly.
...
begin
  ...
  if p_payment_method not in ('bank_transfer', 'promptpay', 'cod') then
  ...
  -- The COD service fee is part of what the shop charges, so it sits inside
  -- the VAT base alongside delivery rather than being bolted on after tax.
  v_cod := case when p_payment_method = 'cod' then public.calc_cod_fee() else 0 end;
  v_vat := public.calc_vat(v_subtotal - v_discount + v_shipping + v_cod);
  v_total := v_subtotal - v_discount + v_shipping + v_cod + v_vat;

  -- The ceiling applies to the final total, because that is the cash the
  -- courier actually has to collect at the door -- not the pre-tax goods
  -- value. Checked server-side: the browser cannot compute this total.
  if p_payment_method = 'cod' and v_total > public.cod_max_total() then
    raise exception 'เก็บเงินปลายทางรับได้ไม่เกิน % บาทต่อคำสั่งซื้อ', public.cod_max_total()
      using errcode = '22023';
  end if;

  insert into public.orders (
    user_id, address_id, shipping_address, customer_name, customer_phone, customer_email,
    customer_note, subtotal, discount_total, shipping_fee, cod_fee, vat_total, total,
    promo_code, business_name, tax_id, branch_name, payment_method
  ) values (
    v_uid, p_address_id, v_ship, v_name, v_phone,
    (select email from public.profiles where id = v_uid),
    p_note, v_subtotal, v_discount, v_shipping, v_cod, v_vat, v_total,
...
-- cod_fee joins the frozen set for the same reason vat_total did: it is part
-- of what the buyer agreed to, and an admin lifecycle update must not rewrite it.

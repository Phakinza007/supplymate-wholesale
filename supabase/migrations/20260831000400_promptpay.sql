-- Static PromptPay: the shop shows one fixed QR, the buyer scans it, types the
-- amount themselves and uploads a slip — the same verification path as a bank
-- transfer. No gateway, no webhook, no auto-confirmation.
--
-- The method is recorded because the buyer chose it and the order history
-- shows it, not because it changes how a slip is verified.
alter table public.orders
  drop constraint orders_payment_method_check,
  add constraint orders_payment_method_check
    check (payment_method in ('bank_transfer', 'promptpay'));

-- The 6-argument overload must go, or the 7-argument one below is ambiguous
-- to PostgREST when the client omits the new parameter.
drop function if exists public.create_order(jsonb, uuid, jsonb, text, text, jsonb);
...
-- payment_method is part of what the buyer submitted, so it joins the frozen
-- set: an admin lifecycle update must never rewrite how they said they paid.

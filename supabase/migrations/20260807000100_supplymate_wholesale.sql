...
-- The business snapshot is captured exactly once at checkout. The function
-- still owns address authorization, product/variant validation, row locking,
-- pricing, discounts, shipping, and all order-item snapshots.
drop function if exists public.create_order(jsonb, uuid, jsonb, text, text);
...
begin
  ...
  -- Resolve the shipping snapshot from an address owned by the caller, or
  -- validate the caller-provided one-off address.
  ...
  -- Price the cart server-side and lock every product row. A requested
  -- variant must be active and belong to the product, and pack minimums are
  -- enforced here rather than trusted to the browser.
  ...
  -- Re-validate promotions under a row lock at order creation. The earlier
  -- validate_promo_code() response is UI feedback only and is never trusted.
...
-- Business identity is an order-time snapshot. Admin lifecycle updates can
-- change operational fields, but can never rewrite what the buyer submitted.
...
-- Preserve the existing optional payment-note contract. Calling this RPC
-- with only order id and path remains valid because p_note keeps its default.
-- A replacement slip clears the prior rejection explanation only after all
-- existing path, ownership, and pending-status checks succeed.
...
revoke execute on function public.attach_payment_slip(uuid, text, text)
  from public, anon;
grant execute on function public.attach_payment_slip(uuid, text, text)
  to authenticated;
